...
for (let i = 0; i < allFiltersChildren.length; i++) {
  allFiltersChildren[i].addEventListener("click", function (e) {
    ...
    if(e.currentTarget.parentNode.classList.contains("selected-clr")){
      e.currentTarget.parentNode.classList.remove("selected-clr");
      loadTasks();
    }else{
      for (let j = 0; j < allFiltersChildren.length; j++) allFiltersChildren[j].parentNode.classList.remove("selected-clr");
      e.currentTarget.parentNode.classList.add("selected-clr");
